...
member.roles.set(conf.erkekRolu)
...
message.channel.send(reawEmbed.setDescription(`${client.emojis.cache.get(a.yes)} ${member} kullanıcısı ${conf.erkekRolu.map(qwe => `<@&${qwe}>`).join(", ")} rolleriyle **erkek** olarak kaydoldu ve ismi \`${a.Taglar.some(ewq => member.user.tag.includes(ewq)) ? `Fix` : `•`} ${isim} | ${yaş}\` olarak değiştrildi.`))
